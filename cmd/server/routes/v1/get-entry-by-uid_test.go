...
func setupGetEntryByUid() (*httptest.ResponseRecorder, func()) {
...
func TestGetEntryByUidSuccess(t *testing.T) {
	rr, teardown := setupGetEntryByUid()
	defer teardown()

	req, _ := http.NewRequest("GET", "/entry-by-uid?uid=test&locale=en", nil)

	GetEntryByUid(rr, req)
	assert.Equal(t, http.StatusOK, rr.Code)
	assert.JSONEq(t, `{"data":{"alt_locales":[],"breadcrumbs":[],"entry":{}},"error":null}`, rr.Body.String())
}

func TestGetEntryByUidErrMissingParams(t *testing.T) {
	rr, teardown := setupGetEntryByUid()
	defer teardown()

	req, _ := http.NewRequest("GET", "/entry-by-uid", nil)

	GetEntryByUid(rr, req)
	assert.Equal(t, http.StatusBadRequest, rr.Code)
	assert.JSONEq(t, `{"data":null,"error":"missing required query params: (uid, locale)"}`, rr.Body.String())
}

func TestGetEntryByUidErrApiGetEntry(t *testing.T) {
	rr, teardown := setupGetEntryByUid()
	defer teardown()
...
func TestGetEntryByUidErrApiGetMetadata(t *testing.T) {
	rr, teardown := setupGetEntryByUid()
	defer teardown()
...
func TestGetEntryByUidErrJson(t *testing.T) {
	rr, teardown := setupGetEntryByUid()
	defer teardown()
